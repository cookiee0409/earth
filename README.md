...
- **회전** — 마우스/터치로 드래그해 지구본을 돌립니다(자동 회전 없음).
- **확대·축소** — 마우스 휠, 화면 우측 하단의 `+` / `−` 버튼, 또는 모바일 핀치 제스처로 줌인·줌아웃.
- **초기화** — `⟳` 버튼으로 초기 위치·배율로 되돌립니다.
- **항공 노선** — 주요 국제선 대권 항로를 따라 작은 비행기 아이콘이 날아갑니다. 하단에서 슬라이더 또는 **날짜 직접 입력**으로 날짜를 바꾸면 **그 날의 운항량만큼** 비행기 수가 늘고 줄며, `▶`로 자동 재생(**1배속 = 하루 5초**)하고 `1× 2× 4× 8× 16×` 배속으로 속도를 조절합니다. 기간은 2019-01-01 ~ **오늘**까지.
...
- **비행 경로 선 + 클릭** — 표시 중인 노선은 두껍고 옅은 대권 곡선으로 그려지고, 경로를 **클릭하면 우측 상단에 대략적인 운항 기록 수**(항공사·노선 수 기준)가 팝업으로 표시됩니다.
- **국가별 색상 + 공항 마커** — 비행기·경로 색은 출발 국가 색을 따릅니다(기본값은 대륙 색). 국가 패널의 색상 칸을 클릭하면 팔레트가 열려 **국가별 임의 색**을 지정할 수 있고, 선택한 국가의 **공항 위치는 링 마커**로 표시됩니다. 한 대륙에서 일부 국가만 선택하면 해당 대륙 칸이 음영으로 표시됩니다. 더 깊은 줌(한 국가가 화면을 꽉 채울 정도)도 지원합니다.
...
### 한국 노선 정확도 (공공데이터 연동 방법)

`scripts/process-routes.js`의 `KR_ROUTES`는 OpenFlights가 놓치는 국내선(예: GMP–CJU, 세계 1위 노선)을 **실제 일일 운항 빈도 근사치**로 보강합니다. 더 정확히 하려면 **공공데이터포털(data.go.kr)** API를 연동하세요(서비스 키 필요):

| 공항 | 용도 | API |
|---|---|---|
| 인천(ICN) | 정기 운항 스케줄 | 인천국제공항공사 **정기 여객편 운항 현황 상세 조회** |
| 인천(ICN) | 실제 운항 상태 | 인천국제공항공사 **항공기 운항 현황 상세 조회** (D-3~D+6) |
| 김포·김해·제주·대구 등 14개 | 운항 스케줄 | 한국공항공사(KAC) **항공기 운항 스케줄** |
| KAC 공항 | 실시간 운항 | 한국공항공사 **실시간 항공기 운항정보** (D-3~D+6) |

연동 방식: data.go.kr에서 서비스 키 발급 → Vercel 환경변수(`DATA_GO_KR_KEY`)에 저장 → 서버리스 함수(`api/kr-flights.js`)에서 스케줄을 받아 노선별 빈도로 집계 → 클라이언트가 `routes.json`의 한국 노선을 그 값으로 대체. (서비스 키가 없으면 위 근사치를 사용합니다. 키를 주시면 이 연동을 구현해 드립니다.)
