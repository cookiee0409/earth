...
- **대륙별 색상** — 출발 대륙(LIVE 모드에서는 현재 위치 대륙)에 따라 비행기 색이 달라집니다. 좌상단 범례 참고.
- **LIVE 실측 모드** — `LIVE` 버튼을 누르면 [OpenSky Network](https://opensky-network.org/)에서 **지금 하늘을 나는 실제 항공기**를 받아 실제 좌표·방위·속도로 표시하고, 현재 비행 중인 대수(실측)를 보여줍니다. (OpenSky는 유럽 수신기 비중이 높아 유럽 상공이 가장 조밀합니다.)
- **화면 크기 자동 조절** — 뷰포트 넓이에 맞춰 노선 수(350~1,600)와 비행기 상한(90~560대)을 자동 조정해, 작은 화면·저사양 기기에서도 부드럽게 동작합니다.
...
`LIVE` 버튼은 Vercel 서버리스 함수 [`api/live.js`](api/live.js)를 호출합니다. 이 함수는 OpenSky Network `/states/all`(현재 추적 중인 전 세계 항공기)을 받아 공중에 있는 기체만 추려 샘플링·대륙 분류해 반환합니다. 엣지 캐시(`s-maxage`)와 웜 람다 메모리 캐시로 OpenSky 호출을 공유해 요청 제한을 보호합니다.

- **익명**으로도 동작하지만 요청 제한이 빡빡합니다.
- **권장**: [OpenSky 계정](https://opensky-network.org/)에서 API 클라이언트를 만들고 Vercel 환경변수에 `OPENSKY_CLIENT_ID`, `OPENSKY_CLIENT_SECRET`를 설정하면 OAuth2로 더 안정적으로 동작합니다(설정 시 자동 사용).
